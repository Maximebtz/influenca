...
function Header() {
  ...
  return (
    <div className='relative'>
      ...
      <div id='user-dropdown-menu' className={`absolute top-full translate-y-1/2 mt-2 px-2 right-4 w-max h-max rounded-md border border-zinc-200 drop-shadow-lg transition-all duration-100 bg-white z-10 ${isUserDropdownMenuOpen ? 'block' : 'hidden'}`}>
        {session && session.user.role === 'INFLUENCER' && (
          <div>
            <a href="/boutique/products">
              <button className='p-2 text-nowrap bg-transparent mt-1 text-black gap-2'>
                Boutique
              </button>
              <hr className='border-t w-full opacity-80 bg-influenca-gray' />
            </a>
        ...
        <button onClick={() => signOut()} className='p-2 text-nowrap bg-transparent mt-0 text-black flex items-center gap-2'>
